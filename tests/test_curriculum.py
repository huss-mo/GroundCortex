...
class TestCurriculumManager:
    ...
    def test_regularization_always_included(self, db):
        ...
        cm = CurriculumManager(db)
        _, all_rows, _val = cm.build("run-1")
    ...
    def test_trained_experiences_use_cached_example(self, db):
        ...
        cm = CurriculumManager(db)
        _, all_rows, _val = cm.build("run-2")
    ...
    def test_new_examples_get_run_id(self, db):
        ...
        cm = CurriculumManager(db)
        _, all_rows, _val = cm.build("my-run")
    ...
    def test_regularization_rows_get_current_run_id(self, db):
        _add_exp(db, "A fact.")
        cm = CurriculumManager(db)
        _, all_rows, _val = cm.build("my-run-id")
        reg_rows = [r for r in all_rows if r.variant == "regularization"]
    ...
    def test_val_examples_returned_not_saved(self, db):
        exp = _add_exp(db, "New fact.")
        cm = CurriculumManager(db)
        _, _, val = cm.build("placeholder")
        assert len(val) == 1
        assert val[0].variant == "validation"
        assert val[0].run_id == "placeholder"
        # not persisted yet - caller is responsible
        assert db.get_validation_examples([exp.id]) == []

    def test_existing_val_examples_not_duplicated(self, db):
        exp = _add_exp(db, "Fact.", status="pending")
        from groundcortex.pipeline.models import TrainingExample
        existing = TrainingExample(run_id="old-run", experience_id=exp.id,
                                   variant="validation", messages=[])
        db.save_training_examples([existing])
        cm = CurriculumManager(db)
        _, _, val = cm.build("new-run")
        assert val == []
